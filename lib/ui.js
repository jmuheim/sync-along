...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .bookmarklet-wrap { position: relative; }
    .bookmarklet-wrap textarea { width: 100%; background: #0f3460; color: #a8b2d8; border: none; border-radius: 8px; padding: 0.75rem 0.75rem 2.5rem; font-size: 0.75rem; font-family: monospace; resize: none; height: 80px; outline: none; }
    .bookmarklet-wrap button { position: absolute; bottom: 0.5rem; right: 0.5rem; background: #e94560; color: white; border: none; padding: 0.25rem 0.65rem; border-radius: 5px; font-size: 0.75rem; cursor: pointer; }
    .bookmarklet-wrap button:hover { background: #c73652; }
...
<body>
  ...
  <div class="card">
    ...
    <a id="bm-link" class="bookmarklet" href="${bookmarklet}">Sync Along</a>
    ...
    <div class="bookmarklet-wrap">
      <textarea id="bm-text" readonly></textarea>
      <button onclick="(function(btn){var ta=document.getElementById('bm-text');if(navigator.clipboard){navigator.clipboard.writeText(ta.value).then(function(){btn.textContent='Copied!'});}else{ta.select();document.execCommand('copy');btn.textContent='Copied!';}})(this)">Copy</button>
    </div>
  ...
  <script>document.getElementById('bm-text').value = document.getElementById('bm-link').getAttribute('href');</script>
</body>
</html>`;
}

export function buildDevHTML(ip, port, demoPages) {
  const stub = buildStubBookmarklet(ip, port);
  const stubCode = decodeURIComponent(stub.replace('javascript:', ''));

  const sidebarLinks = demoPages.map(name => {
    const label = name.replace(/\.html$/, '').replace(/-/g, ' ');
    return `<a href="#" class="demo-link" data-src="/demos/${encodeURIComponent(name)}">${escapeHTML(label)}</a>`;
  }).join('\n');
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    body { height: 100%; overflow: hidden; display: flex; flex-direction: column; font-family: system-ui, sans-serif; background: #1a1a2e; color: #eee; }
    #topbar {
    ...
    #bookmarklet-link {
      ...
      font-size: 0.95rem; font-weight: bold; text-decoration: none; white-space: nowrap;
    }
    #bookmarklet-link:hover { background: #c73652; }
...
<body>
  <div id="topbar">
    ...
    <a id="bookmarklet-link" href="${stub}" title="Drag to save · click to run in frame">Sync Along</a>
  </div>
  <div id="layout">
    <nav id="sidebar">
  ...
  <script>
    const STUB_CODE = ${JSON.stringify(stubCode)};
    ...
    bmLink.addEventListener('click', e => {
      ...
      e.preventDefault();
      try {
        frame.contentWindow.eval(STUB_CODE);
      } catch(err) {
        alert('Could not run bookmarklet in frame: ' + err.message);
      }
    });
  </script>
</body>
