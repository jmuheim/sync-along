...
export function buildBookmarkletSource(wsURL, clientScript) {
  ...
  function sendPage(el){
    ...
    var elWidth=0;
    if(el!==document.body){var sw=el.style.width;el.style.width='max-content';elWidth=Math.max(Math.round(el.getBoundingClientRect().width),Math.round(window.innerWidth/2));el.style.width=sw;}
    lastElWidth=elWidth;
    ...
    var html='<!DOCTYPE html><html>'
      +headHTML.replace(/<\\/head>/i,
        ...
        +'<style>html{scrollbar-width:none!important}html::-webkit-scrollbar{display:none!important}body{margin:0!important}</style>'
  ...
  function updateClientBars(){
    ...
    Object.keys(clients).forEach(function(id){
      var c=clients[id];
      var clientContentH=(lastElWidth>0&&c.width)?c.height*lastElWidth/c.width:c.height;
      var frac=Math.min(1,clientContentH/vMasterContent);
