...
export function buildClientScript(wsURL) {
  return `(function(){if(window.__circleSyncClient)return;window.__circleSyncClient=true;if(window.__circleSyncClientWS){window.__circleSyncClientWS.close();window.__circleSyncClientWS=null;}var lastRatio=-1;var ws=null;var resizeTimer=null;function sendViewport(){if(ws&&ws.readyState===1)ws.send(JSON.stringify({type:'viewport',height:window.innerHeight}));}window.addEventListener('resize',function(){clearTimeout(resizeTimer);resizeTimer=setTimeout(sendViewport,300);});function connect(){ws=new WebSocket('${wsURL}?role=client');ws.onopen=function(){sendViewport();};ws.onmessage=function(e){var m=JSON.parse(e.data);if(m.type==='page'){lastRatio=-1;document.open();document.write(m.html);document.close();window.scrollTo(0,0);}else if(m.type==='scroll'){if(m.ratio!==lastRatio){var down=m.ratio>lastRatio;lastRatio=m.ratio;var scrollable=Math.max(1,document.documentElement.scrollHeight-window.innerHeight);var clientRatio=window.scrollY/scrollable;if((down&&m.ratio>clientRatio)||(!down&&m.ratio<clientRatio))window.scrollTo(0,m.ratio*scrollable);}}else if(m.type==='requestViewport'){sendViewport();}};ws.onclose=function(){setTimeout(connect,2000);};}connect();})();`;
}
...
export function buildBookmarkletSource(wsURL, clientScript) {
  ...
  function updateClientBars(){
    ...
    var iframeVpH=masterOverlay?masterOverlay.contentWindow.innerHeight:screenH;
    ...
    Object.keys(clients).forEach(function(id,i){
      ...
      var thumbH=frac*screenH;
      var thumbTop=currentRatio*(screenH-thumbH);
      var color=CLIENT_COLORS[i%CLIENT_COLORS.length];
      var track=document.createElement('div');
      Object.assign(track.style,{width:'8px',height:'100%',position:'relative',background:'rgba(0,0,0,0.25)',overflow:'hidden'});
  ...
  ws.onmessage=function(e){
    ...
    if(m.type==='viewport'){
      clients[m.clientId]={height:m.height};
      updateClientBars();
