...
export function buildClientScript(wsURL) {
  return `(function(){if(window.__circleSyncClient)return;window.__circleSyncClient=true;function connect(){var ws=new WebSocket('${wsURL}?role=client');ws.onmessage=function(e){var m=JSON.parse(e.data);if(m.type==='page'){document.open();document.write(m.html);document.close();}else if(m.type==='scroll'){window.scrollTo(0,m.y);}};ws.onclose=function(){setTimeout(connect,2000);};}connect();})();`;
}
...
export function buildBookmarkletSource(wsURL, clientScript) {
  ...
  function sendScroll(){
    if(ws.readyState===ws.OPEN)ws.send(JSON.stringify({type:'scroll',y:window.scrollY}));
  }
  ...
  function startPickMode(){
    ...
    function onPick(e){
      ...
      if(shareBtn.parentNode)shareBtn.remove();
      saveSel(el);
      if(ws.readyState===ws.OPEN){sendPage(el);installScroll();}
      else ws.addEventListener('open',function once(){ws.removeEventListener('open',once);sendPage(el);installScroll();});
    }
