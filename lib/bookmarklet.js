...
export function buildBookmarkletSource(wsURL, clientScript) {
  ...
  function updateBand(){
    if(!masterOverlay||!bandEl)return;
    var scrollTop=masterOverlay.contentWindow.scrollY;
    var scrollHeight=masterOverlay.contentDocument.documentElement.scrollHeight;
    var masterVpH=masterOverlay.contentWindow.innerHeight;
    var ratio=scrollTop/Math.max(1,scrollHeight-masterVpH);
    var top=+(ratio*0.6).toFixed(3);
    var bot=+((1-ratio)*0.6).toFixed(3);
    bandEl.style.background='linear-gradient(to bottom,rgba(0,0,0,'+top+') 0%,transparent 60%),linear-gradient(to top,rgba(0,0,0,'+bot+') 0%,transparent 60%)';
    if(arrowEl)arrowEl.style.top=(ratio*window.innerHeight)+'px';
  }

  function createBand(){
    if(bandEl)return;
    bandEl=document.createElement('div');
    Object.assign(bandEl.style,{position:'fixed',top:0,left:0,width:'100%',height:'100%',zIndex:2147483645,pointerEvents:'none',visibility:'visible'});
    document.body.appendChild(bandEl);
    arrowEl=document.createElement('div');
    arrowEl.textContent='◄';
    Object.assign(arrowEl.style,{
      position:'fixed',right:'0.3rem',zIndex:2147483645,pointerEvents:'none',visibility:'visible',
      fontSize:'6rem',lineHeight:'1',color:'#FFE600',
  ...
  function sendScroll(){
    ...
    ws.send(JSON.stringify({type:'scroll',ratio:scrollTop/Math.max(1,scrollHeight-viewportHeight)}));
    updateBand();
  }
