...
export function buildClientScript(wsURL) {
  return `(function(){if(window.__circleSyncClient)return;window.__circleSyncClient=true;var lastRatio=-1;function sendViewport(ws){ws.send(JSON.stringify({type:'viewport',height:window.innerHeight,scrollHeight:document.documentElement.scrollHeight}));}function connect(){var ws=new WebSocket('${wsURL}?role=client');ws.onopen=function(){sendViewport(ws);};ws.onmessage=function(e){var m=JSON.parse(e.data);if(m.type==='page'){lastRatio=-1;document.open();document.write(m.html);document.close();window.scrollTo(0,0);}else if(m.type==='scroll'){if(m.ratio!==lastRatio){var down=m.ratio>lastRatio;lastRatio=m.ratio;var scrollable=Math.max(1,document.documentElement.scrollHeight-window.innerHeight);var clientRatio=window.scrollY/scrollable;if((down&&m.ratio>clientRatio)||(!down&&m.ratio<clientRatio))window.scrollTo(0,m.ratio*scrollable);}}else if(m.type==='requestViewport'){sendViewport(ws);}};ws.onclose=function(){setTimeout(connect,2000);};}connect();})();`;
}
...
export function buildBookmarkletSource(wsURL, clientScript) {
  ...
  function sendPage(el){
    ...
    lastElWidth=elWidth;
    var viewport=elWidth>0?'width='+elWidth:'width=device-width,initial-scale=1';
  ...
  function updateClientBars(){
    ...
    Object.keys(clients).forEach(function(id,i){
      var c=clients[id];
      var vpFrac=c.scrollHeight>c.height?c.height/c.scrollHeight:1;
      var thumbH=vpFrac*window.innerHeight;
      var thumbTop=currentRatio*(1-vpFrac)*window.innerHeight;
      var color=CLIENT_COLORS[i%CLIENT_COLORS.length];
      var track=document.createElement('div');
      Object.assign(track.style,{width:'8px',height:'100%',position:'relative',background:'rgba(0,0,0,0.25)'});
      var thumb=document.createElement('div');
      Object.assign(thumb.style,{position:'absolute',left:'0',width:'100%',height:thumbH+'px',top:thumbTop+'px',background:color,opacity:'0.85',borderRadius:'3px'});
  ...
  function createClientBars(){
    ...
    Object.assign(clientBarsEl.style,{position:'fixed',right:'0',top:'0',height:'100%',display:'flex',flexDirection:'row',alignItems:'stretch',pointerEvents:'none',zIndex:'2147483645',visibility:'visible'});
  ...
  ws.onmessage=function(e){
    ...
    if(m.type==='viewport'){
      clients[m.clientId]={height:m.height,scrollHeight:m.scrollHeight};
      updateClientBars();
