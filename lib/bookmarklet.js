...
export function buildStubBookmarklet(ip, port) {
  ...
  const code = `(function(){fetch('${url}').then(function(r){return r.text()}).then(eval)})()`;
...
export function buildClientScript(wsURL) {
  return `(function(){if(window.__circleSyncClient)return;window.__circleSyncClient=true;var lastRatio=-1;function connect(){var ws=new WebSocket('${wsURL}?role=client');ws.onmessage=function(e){var m=JSON.parse(e.data);if(m.type==='page'){lastRatio=-1;document.open();document.write(m.html);document.close();window.scrollTo(0,0);}else if(m.type==='scroll'){if(m.ratio!==lastRatio){var down=m.ratio>lastRatio;lastRatio=m.ratio;var clientRatio=window.scrollY/Math.max(1,document.documentElement.scrollHeight);if((down&&m.ratio>clientRatio)||(!down&&m.ratio<clientRatio))window.scrollTo(0,m.ratio*document.documentElement.scrollHeight);}}};ws.onclose=function(){setTimeout(connect,2000);};}connect();})();`;
}
